...
NTL.phys.data.y1999 <- filter(NTL.phys.data, year4 %in% c('1999'))
#year4==1999
...
NTL.phys.data.tues90to99 <- filter(NTL.phys.data.tuesdaylake, year4 %in% c(1990:1999))
#lakename=='Tuesday lake' & year4 >= 1990 & year4 <= 1999
...
(lakename == 'Tuesday Lake' & year4 >= 1990 & year4 <= 1999 & month(sampledate) == 7)
...
pivot_longer(irradianceWater:irradianceDeck, names_to = 'irradiance', values_to = 'iradiation')

#Exercise 6: spread temperatureC into more than one column based on the depth
NTL.phys.data.Ex6 <- NTL.phys.data %>% picot_wider(names_from = 'depth', values_from = 'temperature_C')
```
